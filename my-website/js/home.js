...
let allMovies = [], allTVShows = [], allAnime = [];
...
function populateYearFilter(items, selectId) {
  const years = new Set();
  items.forEach(item => {
...
async function init() {
  ...
  displayList(allAnime, 'anime-list');

  populateYearFilter(allMovies, 'movie-year-select');
  populateYearFilter(allTVShows, 'tvshow-year-select');

  document.getElementById('movie-year-select').onchange = (e) => {
    displayList(filterByYear(allMovies, e.target.value), 'movies-list');
  };
  document.getElementById('tvshow-year-select').onchange = (e) => {
    displayList(filterByYear(allTVShows, e.target.value), 'tvshows-list');
  };
}
